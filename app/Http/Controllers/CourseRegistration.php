...
class CourseRegistration extends Controller
{
    ...
    public function departmentCourses(Request $request, $id)
    {
        ...
        $dept_courses = Department::find($id)->courses()
            ->where('level', $level)
            ->where('semester', $fetch_semester)->get(); //object

        // Getting UB requirements
        if ($level < 500) {
            $ub_req = Course::where('department_id', null)
                ->where('level', null)
                ->where('semester', $fetch_semester)
                ->orWhere('semester', null)
                ->get();
                
                // Pushing UB requirements into department courses
                foreach ($ub_req as $ub) {
                    $dept_courses[] = $ub;
                }
                // Endof the act
        }
    ...
    public function getExamResults()
    {
        ...
        if (
            Schema::connection('reg_courses')->hasTable($table) &&
    ...
    public function generateCourseList($code)
    {
        ...
        foreach ($tables as $table) {
            $names[] = $table->Tables_in_cot_students; //depends on the name of the database.
        }
    ...
    public function generateStudentList()
    {
        ...
        $students = [];
        if (strtolower($cert) === 'btech') {
            if ($level === 'all' && $dept_id !== 'all') {
                $dept_name = Department::find($dept_id)->name;
                $students = User::where('department', $dept_name)
                    ->where('certificate', 'BTech')->get(['name', 'matricule', 'level', 'department', 'option']);
            } else if ($level !== 'all' && $dept_id === 'all') {
                $students = User::where('certificate', 'BTech')
                    ->where('level', $level)
                    ->get(['name', 'matricule', 'level', 'department', 'option']);
            } else {
                $students = User::where('certificate', 'BTech')
                    ->get(['name', 'matricule', 'level', 'department', 'option']);
            }
            ...
            if ($level === 'all' && $dept_id !== 'all') {
                $dept_name = Department::find($dept_id)->name;
                $students = User::where('department', $dept_name)
                    ->where('certificate', 'MTech')->get(['name', 'matricule', 'level', 'department', 'option']);
            } else if ($level !== 'all' && $dept_id === 'all') {
                $students = User::where('certificate', 'MTech')
                    ->where('level', $level)
                    ->get(['name', 'matricule', 'level', 'department', 'option']);
            } else {
                $students = User::where('certificate', 'MTech')
        ...
        foreach ($students as $std) {
            $std->department = Department::where('name', $std->department)->first()->slogan;
        }
