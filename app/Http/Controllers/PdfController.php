...
class PdfController extends Controller
{
    public function registrationReceipt($matricule)
    {
        ...
        if ($student->level == 200) {
            $deg_txt .= 'an under graduate (First year)';
        } else if ($student->level == 400) {
            $deg_txt .= 'an under graduate (Third year)';
        } else {
            $deg_txt .= 'a post graduate (Masters)';
        }
        $data = [
            'name' => $student->name,
            ...
            'option' => strtolower($student->option),
    ...
    public function transaction($matricule)
    {
        ...
        if ($transaction->type == 1) {
            $str_type .= 'Tuition (school) fee';
        } else if ($transaction->type == 2) {
            $str_type .= 'Medical fee';
        }
        $data = [
            'name' => $student->name,
            'date_of_birth' => date('jS\ F Y', strtotime($student->date_of_birth)),
        ...
        $file_name = "$matricule $str_type receipt for $academic_year.pdf";
